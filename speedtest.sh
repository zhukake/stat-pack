...
speed_test() {
    local nodeName="$2"
    if [ -z "$1" ];then 
        ./speedtest-cli/speedtest --progress=no --accept-license --accept-gdpr >./speedtest-cli/speedtest.log 2
>&1
    else
        ./speedtest-cli/speedtest --progress=no --server-id="$1" --accept-license --accept-gdpr >./speedtest-cl
i/speedtest.log 2>&1
    fi
    if [ $? -eq 0 ]; then
...
_speed}" "${dl_speed}" "${latency}"
        fi
    fi
}

#2716 Russian Federation,Rostov-on-Don,DOM.RU,speedtest.rostov.ertelecom.ru:8080,2716
#Russian Federation,Rostov-On-Don,Elit-TV,speedtest.elit-tv.ru:8080,16261
#Russian Federation,Sochi,VIP Sochi,speedtest1.sch.beeline.ru:8080,6210
...
speed() {
    ...
    speed_test '2716' 'Rostov-on-Don,DOM.RU'
    speed_test '16261' 'Rostov-On-Don,Elit-TV'
    speed_test '6210' 'Sochi,beeline.ru'

}

io_test() {
    (LANG=C dd if=/dev/zero of=benchtest_$$ bs=512k count="$1" conv=fdatasync && rm -f benchtest_$$) 2>&1 | awk
 -F, '{io=$NF} END { print io}' | sed 's/^[ \t]*//;s/[ \t]*$//'
}
...
get_system_info() {
    ...
    in_kernel_no_swap_used_size=$(
        LANG=C
        df -t simfs -t ext2 -t ext3 -t ext4 -t btrfs -t xfs -t vfat -t ntfs --total 2>/dev/null | grep total | 
awk '{ print $3 }'
    )
